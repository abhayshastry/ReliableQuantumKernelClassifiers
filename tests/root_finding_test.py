...
C = 4 
def stochastic_root_finding(f, x_range = (1, 1000), N_queries = 20, N_trials = 1000, delta = 0.1, domain_f = "Int"):
    """
...
def find_N_star(args, N_trials = 100,  delta = 0.1):
    ...
    print( f"N_star found by root finding:{A}")
    F = [fn1(A[0]) + margin_err  for _ in range(N_trials)]
    return F




def N_vs_R_emp_plot(args, N_list = np.linspace(1,200,15), N_trials = 100,  delta = 0.1):
    key, m, C = args
    print(f'[JOB {args}]', flush=True)
    K_train, y_train = return_kernel(key, m )
    if C == "optimal":
    ...
    plot_list = []
    for x in N_list:
        A = [fn1(x) for  _ in range(N_trials)]
        plot_list.append( high_prob_upper(A, delta))
    plt.plot(N_list, plot_list)
    plt.show()
    return 


args = (kd_list[9], 120, 1.0)
List = find_N_star(args)
N_vs_R_emp_plot(args)
