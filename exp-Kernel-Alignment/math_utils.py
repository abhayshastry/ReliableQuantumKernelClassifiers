...
def primal_robust_socp(K_star, y_train, delta_1 = 0.1, delta_2 = 0.1, C = np.inf, shots = 20, circuit_type = 'gates'):
    n = K_star.shape[1]
    N = 2*n + 2
    g = 1
    if circuit_type == 'swap':
    ...
    Ai = np.zeros((n,N))
    Ai[:n,:n] = np.eye(n)
    Ai[:n,n:2*n]= np.multiply(y_train,K_star).T
    Ai[:n,N-2] = y_train
    Ai[:n,N-1] = -kappa(delta_1)*(g/np.sqrt(shots))*np.ones(n)
        #linear_constraints += x[i]>= 1 - y_train[i]*(np.dot(x[n:2*n],K_star[:,i]) + x[2*n]) + kappa(delta_1)*(g/np.sqrt(shots))*x[N-1]
    ...
    return beta, b, primal_optimal_value
