...
#        return angle_kernel(x,y,qubits)
    ...
    qml.inv(QAOAEmbedding(y,weights, wires = range(qubits)))
...
#    qubits = len(dev_kernel.wires)
    ...
    qml.inv(AngleEmbedding(x2, wires=range(qubits)))
    return qml.expval(qml.Hermitian(P,wires=range(qubits)))
...
#    qubits = len(dev_kernel.wires)
    ...
    qml.inv(IQPEmbedding(y, wires = range(qubits), n_repeats = n_repeats))
